...
```mermaid
flowchart TD
    A[Raw EEG Input] --> B["Phase 0: SSL Pretraining<br/>Shared 64-dim embedding space"]
    B --> C["Cross-User Generalization<br/>'Who is this person?'<br/>SSRS + FiLM conditioning"]
    B --> D["Cross-Session Generalization<br/>'How is the signal drifting now?'<br/>DTKF: diagonal Kalman filter"]
    C --> E["Calibration-Free Prediction<br/>No adversarial training · under 50K params"]
    D --> E
```
...
```mermaid
flowchart TD
    A["Raw EEG (C_in × T_native)<br/>tagged: dataset, subject, session"] --> B["Resample → 250 Hz<br/>anti-aliased, cached once"]
    B --> C["CAR — Common Average Reference<br/>per-dataset flag, logged"]
    C --> D["Notch Filter — 50/60 Hz + 2nd harmonic<br/>per-dataset, by recording country"]
    D --> E["Trial / Epoch Extraction<br/>cue-aligned, fixed length L"]
    E --> F["MAD Artifact Clipping<br/>per-epoch, % clipped logged"]
    F --> G["Robust Amplitude Normalization<br/>per-epoch median/MAD"]
    G --> H[["CACHE WRITTEN<br/>cache/&lt;dataset&gt;/subject_N.npz"]]
    H --> I["Subject-Balanced Batch Sampling<br/>sampler.py — P×K, ≥2 clips/subject"]
    I --> J["Augmentation ×2 views<br/>time_shift → amplitude_scale → gaussian_noise →<br/>channel_dropout → time_mask → temporal_crop → freq_mask"]
    J --> K["LSP: Per-Dataset Channel Mixer<br/>lsp.py — (C_dataset,T) → (16,T)"]
    K --> L["EEGNet 9-Band Spectral Encoder<br/>encoder.py — ~17.3K params → (8,64)"]
    L --> M["Projection Head<br/>projection_head.py — 2-layer MLP → 64-dim"]
    M --> N["InfoNCE Loss, Subject-Balanced Negatives<br/>losses.py"]
    N --> O["train_phase0.py<br/>AdamW, lr=1e-3, until loss plateaus"]
    O --> P[["Output: phase0_checkpoint_&lt;dataset&gt;.pt"]]

    style H fill:#1a1a2e,color:#fff
    style P fill:#1a1a2e,color:#fff
```

*Cache stores post-notch/clip/normalize epochs — LSP and the filter bank are applied at train time, so LSP weights can update without invalidating cached data.*
...
```mermaid
pie showData
    title Phase 0 Build Status (by stage)
    "Done (7 stages)" : 7
    "In progress (LSP 2/4, downloads)" : 2
    "Not started (Gate probes)" : 1
```
